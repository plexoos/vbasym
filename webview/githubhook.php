...
if ((($ip & $mask_27) === ip2long('204.232.175.64')) || (($ip & $mask_22) === ip2long('192.30.252.0')))
{
	...
	if (!$payload) die('no');
	system('git fetch 1>/dev/null 2>/dev/null; git stash 1>/dev/null; git checkout origin/master; git stash pop 1>/dev/null 2>/dev/null; cd /home/cnipol/vbasym_web_production/doc; /usr/local/bin/doxygen 1>/dev/null 2>/dev/null');
	echo 'ok';
}
else
{
