...
for fillNo in `cat $codePath/runlists/MC_list_Wp_2012_numbers.txt`
do

    isMC=350

   echo submitting job for fillNo = $fillNo
    star-submit-template -template $schedMacro/MCWp2012Templ.xml -entities  n1=$mxEve,outPath=$outPath,codePath=$codePath,inpPath=$inpPath,fillNo=$fillNo,isMC=$isMC,STAR_VER=$STAR_VER
done
